...
try {
  ...
  execSync("git submodule update --init --remote --recursive --merge", {
    stdio: "inherit",
  });
} catch (err) {
    ...
    if (fs.existsSync(path.join(appPath, "package.json"))) {
      try {
        ...
        if (fs.existsSync(buildOutput)) {
          ...
          console.error(
            `   ❌ Build successful but 'dist' folder not found in ${appName}`,
          );
      ...
      console.log(
        `   ⚠️  No package.json in ${appName}. Skipping build/deployment.`,
      );
    }
  });
}
...
ghpages.publish(
  ...
  (err) => {
    if (err) {
      ...
      console.log(
        "✅ Deployment complete! Check https://nessdan.github.io/keyboard-web",
      );
    }
  },
);
